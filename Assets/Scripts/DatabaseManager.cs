        ...
        foreach (long index in indices)
        {
            reference.Child(databasePath).Child(index.ToString()).GetValueAsync().ContinueWithOnMainThread(task => {
                ...
                else if (task.IsCompleted)
               {
                    ...
                    // Debug.Log("Online words count : " + OnlineWordsList.Count);
                    // Debug.Log("Indices words count : " + indices.Count);
               }

            //    var downloadTask = reference.GetValueAsync();
            //    float taskProgress = Mathf.Clamp01(downloadTask.Progress);
            ...
            //    Debug.Log("Task Progress :--->>" + taskProgress);
                ...
                {
                    // Debug.Log("Online words count : --Done" + OnlineWordsList.Count);
                    // Debug.Log("Indices words count :--Done " + indices.Count);
                    EnterOnlineGameMode();
                 //  PlayOnSelectLanguage?.Invoke();
                }
